# Changelog

## v5.14 (2026-09-17)

- 强化 Proposal Section 1 三个提示语的语义分工：允许中心问题下有少量受控子问题，同时区分 substantive find-out 内容与学习技能。
- 增加问题范围、方法与证据边界、技能—方法匹配及预期成果比例的检查，防止把建议、行业影响或独立实验承诺写入不相称的计划。
- 明确三个提示语需要可识别但不要求固定段落、开头或句式，并加强 P301 文献研究不误写成 primary measurement 或 causal proof 的规则。

## v5.13 (2026-09-17)

- 增加 Proposal Section 2 的“动机发展叙事”内部检查：从真实兴趣或观察到问题形成、范围收窄和具体项目选择。
- 将 Section 2 的写作路线明确为可选组合，不固定段落顺序、开头或结尾，也不要求每位学生都有戏剧性的触发事件。
- 强化 `[TO CONFIRM]`、不从最终论文反推个人经历，以及在同批文本中保持证据标准一致而让表达路径随学生证据和声音变化。

## v5.12 (2026-09-14)

- 将 Evaluation/Reflection 从固定写作顺序改为“固定评价维度、可变写作路径”。
- 支持根据学生证据选择不同的开头、顺序、段落组织、过渡和结尾，减少同批文本同质化。
- 明确受控变化不能省略必要证据、改变评分标准或为了随机而随机。
- 增加同批评审的公平性边界，区分语言/结构变化与实质性证据判断。

## v5.11 (2026-09-14)

- 增加 Evaluation/Reflection 的成果、研究方法、研究发展和项目管理评价框架。
- 强化“限制—影响—改进—迁移学习”证据链，区分工作框架、支持性证据、机制检验和普遍结论。
- 增加因果强度与结论范围校准，防止把案例、二手资料或相关性写成普遍因果证明。
- 增加自然开头、学生声音和最终稿轻量 proofreading 规则，避免模板化或过度润色。

## v5.10 (2026-09-14)

- 增加 Activity Log 的过程证据与自然语言写作指导。
- 明确区分真实的 next action 与机械重复的 `My next step...` 结尾。
- 强化计划、监控、决策、调整、Supporting Evidence 和学生声音检查。
- 明确 Activity Log 与后续 Evaluation/Reflection 的功能边界，防止事后倒推和无证据的个人成长表述。

## v5.9 (2026-09-11)

- 增加政策、法律、伦理、医学、权利和其他争议性议题的 P301 题目设计与风险检查。
- 增加中性题目压力测试，防止预设结论、把价值判断写成事实、外国制度替代本地分析和法律措辞过度概括。
- 明确题目敏感性本身不能预测成绩；实际判断应回到研究设计、证据质量、反方论证和中心审批要求。
- 将题目修改、来源替换、范围变化和审批风险衔接到 Activity Log。

## v5.8 (2026-09-11)

- 增加 P301 从宽泛议题收窄到可回答研究问题的设计流程。
- 增加 Introduction、Literature Review、Discussion、Conclusion 四部分的高中生友好型论文规划框架。
- 增加段落—证据—局限矩阵和来源链接核验规则，强化定题到 Proposal、Activity Log 的衔接。
- 明确 Literature Review 展示已有证据，Discussion 负责比较、评价并解释证据对研究问题的意义。

## v5.7 (2026-09-11)

- 增加 `What areas of research will you cover?` 专门指导。
- 区分 academic areas、potential related fields、cases 和 source types。
- 支持 Proposal 阶段使用 provisional research areas，并通过 Activity Log 记录范围发展。
- 防止为了凑数量增加无关学科，或提前预设哪种理论解释力更强。

## v5.6 (2026-09-10)

- 增加 Proposal Section 4 / Resources 专门指导。
- 明确 research、write-up、presentation 三类资源功能。
- 增加来源层级、访问方式、用途、局限和问题对齐检查。
- 区分 academic、official、industry、company、news 和 search-tool 的证据角色。
- 防止为了追求来源种类而加入不服务于研究问题的 social media 或泛化 online articles。

## v5.5 (2026-09-10)

- 增加 Proposal Section 3 / Activities and Timescales 专门指导。
- 支持高层阶段与贯穿性任务并行的计划结构。
- 增加时间重叠、依赖、buffer、review point 和真实校历检查。
- 强化 Activity Log、reference management 和关键节点 tutor feedback 的持续记录。
- 防止用最终论文目录倒推 Proposal，或把每项活动错误地视为严格串行。

## v5.4 (2026-09-10)

- 增加 Proposal Section 2 / Reasons for choosing the project 专门指导。
- 明确 Section 1 与 Section 2 的功能分工和重复检查。
- 增加个人动机、学科/未来方向、跨学科联系和理论准确性检查。
- 明确战略行为不等于非理性，并按学生证据校准未来方向的确定程度。

## v5.3 (2026-09-10)

- 增加 Proposal Section 1 的篇幅与完整性判断规则。
- 明确 180–200 词可在信息完整时足够，复杂 P301 可参考 230–270 词。
- 明确上述范围只是内部写作启发式，不能替代实际表格限制或 Edexcel 官方要求。
- 防止为了凑字数扩写成 Introduction，或编造内容填充篇幅。

## v5.2 (2026-09-10)

- 增加 Proposal Section 1 三个表格提示语的显式覆盖门槛。
- 要求分别回答研究问题、希望发现什么、希望学会如何做什么。
- 允许在单文本框中用三个清晰段落组织答案，并对缺失信息使用待确认标记。

## v5.1 (2026-09-10)

- 增加 Proposal Section 1 / Project Objectives 专门指导。
- 明确区分项目开始阶段的计划性表达与论文完成后的总结性表达。
- 保留必要专业术语，同时降低不必要的 dissertation-style meta-language。
- 防止把最终结论、未完成的来源、案例、数据或发现倒写进 Proposal。
- 将 objectives 与后续 Activity Log 的真实研究发展建立衔接。

## v5 (2026-09-10)

- 将仓库从定题阶段扩展为完整的 Edexcel EPQ Coach。
- 增加 Proposal、Activity Log、Evaluation/Reflection 和 P301/P302 论文质量评估流程。
- 增加独立的 `references/`，覆盖定题、Proposal、表格工作流、Edexcel rubric 和本地证据边界。
- 明确使用当前项目指定的 Edexcel/Pearson 证据，并区分本地评估记录与官方要求。
- 将六维可行性评分明确为内部教学比较工具，而不是 Edexcel 官方评分。
- 增加 `agents/openai.yaml`，支持 skill 自动发现和 `$edexcel-epq-coach` 调用。

## v4 (2026-08-19, local iteration)

- 增加候选题的最小证据验证和 6 维可行性评分。
- 明确题目、研究问题和研究目标的区别，并要求至少拆出 2–4 个子问题。
- 增加 Proposal/Activity Log 可复用的定题决策记录。
- 支持 P301/P302/P303/P304 的暂定判断和方法理由。
- 补充题目版本、范围、计划和导师复核边界。

## v3 (2026-06-25)

- 修订 2:方法名改为通用命名(兴趣锚定法 / 三源杂交法 / 四维收窄法),不再绑定具体老师
- 修订 1:触发词加入 "确定研究方向"
- 安全扫描:clean

## v1 (2026-06-25)

- 初始版本
- 3 套方法 + 3 公式 + 5 雷区 + 7 防误操作 + 3 案例
- 触发词:`帮学生定题` / `EPQ 选题` / `用 epq-topic-coach` / `定题辅导`
