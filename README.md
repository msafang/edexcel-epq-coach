...
对于政策、法律、伦理、医学、权利或其他有争议的 P301 议题，skill 会额外检查题目是否预设答案、是否把价值判断写成事实、是否把外国制度当作标准答案，以及是否存在法律措辞、证据、伦理或中心审批风险。它不会因为题目敏感就自动拒绝，也不会保证某个题目一定得高分。
...
即使表格只有一个文本框，也会确保三个功能都能识别，但不会强制固定的段落数量或句式。第一问可以在中心问题下加入少量真正必要的子问题或判断维度；第二问写学生希望弄清楚的实质性问题；第三问写与研究方法和成果相关的技能。skill 还会检查范围是否膨胀、研究方法与证据是否匹配、P301 是否误写成独立实验或因果证明，以及建议或宏大行业影响是否被提前当成结论。缺少学生事实时会标记为待确认。
...
Section 2 会与 Section 1 分工处理：Section 1 回答要研究什么、要发现什么、要学会什么；Section 2 说明学生为什么选择这个项目、它与学科或未来方向如何相关，以及为什么选择这个具体问题。若学生确实经历了由兴趣、观察或一次阅读/课堂讨论形成问题，再经过比较或收窄方向，skill 会把这条动机发展过程作为内部检查，帮助文本呈现“为什么最后是这个项目”；但不会要求每个学生都有戏剧性的触发事件，也不会固定开头、段落顺序或结尾，更不会根据最终论文反向编造个人经历。skill 还会检查个人动机的真实性、学科理论准确性、未来方向的确定程度和与 Section 1 的重复。
...
Evaluation/Reflection 会同时评价成果、研究方法和项目管理，并要求把“限制”写成“限制—对解释或管理的影响—具体改进—预期收益”。它还会检查结论是否超出证据、案例是否被误写成普遍因果证明，以及研究能力和个人成长是否有真实的前后变化支持。评价维度保持一致，但写作入口、段落顺序、收尾方式和句式会根据学生证据自适应，不使用同一套可见模板批量生成。
...
- “这个政策/伦理题目会不会影响 EPQ 评分？请检查题目措辞、证据和审批风险。”
- “检查这个 Proposal 是否可执行。”
...
- `references/sensitive-question-design.md`：政策、法律、伦理等敏感议题的中性题目设计与风险检查
...
- `references/evaluation-reflection-writing.md`：Evaluation/Reflection 的成果、方法、管理、限制和迁移学习规则
