# Anonymous Local Evidence and Source Boundary

Use this reference only when the user asks to learn from the current `F:\EPQ产出` directory or its samples. It is a teaching-pattern summary, not an additional Edexcel mark scheme and not evidence of causation.
...
The strongest local records repeatedly made the following visible:

- A narrow question, a rationale, an ordered plan, milestones, and a dated log that records problems, responses, and plan changes.
- Source variety with academic or official material, a resource-evaluation process such as CRAAP, consistent in-text references, and a bibliography that can be checked against the paper.
- A coherent paper structure that uses headings and figures to support the question. Higher-quality work explains why sources agree or disagree, interprets results rather than merely listing them, considers alternatives, and develops the conclusion.
- An evaluation that names a concrete limitation, explains its effect, and justifies what would change next time. Stronger reflections connect the learning claim to a visible change in search strategy, method, analysis, or writing.

## Recurrent risks to check

- A literature review that paraphrases one source after another without synthesis or an explicit link to the research question.
- A conclusion that repeats earlier paragraphs, introduces new evidence, or claims more certainty than the method supports.
- A generic reflection that lists “time management”, “critical thinking”, or “English improved” without a before/after event and evidence.
- P301 papers that present secondary-source summaries as original empirical findings.
